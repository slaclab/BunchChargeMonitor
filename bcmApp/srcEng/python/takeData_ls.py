...
class StreamHanlder:
    ...
    def __init__(self, directory, name, root, size, timeout=3000000):
        ...
        self.buf = np.getbuffer(np.zeros(size*4), 0, size*4)
    ...
    def readStream(self):
        with self.strm as s:
            nelms = s.read(self.buf, self.timeout)
            if nelms == 0:
...
class TriggerHanlder:
    ...
    def setHwAutoream(self):
        self.triggerHwAutoRearmVal.setVal(np.array([1, 1]))

    def resetHwAutoream(self):
        self.triggerHwAutoRearmVal.setVal(np.array([0, 0]))

# Handle buffer paramerters 
class BufferHanlder:
    ...
    def setSize(self, size):
        sAddr = np.array([0x80000000, 0x90000000, 0xA0000000, 0xB0000000, 0xC0000000, 0xD0000000, 0xE0000000, 0xF0000000])
        eAddr = sAddr + 4*size
        self.endAddr.setVal(eAddr)
        self.startAddr.setVal(sAddr)
        self.daqBufferSize.setVal(np.array([size, size]))
        self.initCmd.execute()
...
def main(argv):
    ...
    for i in (streamBegin, streamEnd-1):
        sh.append(StreamHanlder(directory, "Stream" + str(i), root, numSamples))

    # Set the buffer size
    print("Settiing buffer size...")
    bh.setSize(numSamples)
