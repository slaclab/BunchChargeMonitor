...
cat $IOC_DATA/$1/iocInfo/IOC.pvlist | grep -v "waveform" | grep -v "calc" | grep 'CHRG\|CHRGUNC\|RAWSUM\|CHRGFLOAT\|TOROSTATUS\|CHRGNOTMIT' |  grep 'bo\|mbbo\|ai\|int64in' | cut -f1 -d "," | sed 's/$/ 1 scan/' > $IOC_DATA/$1/archive/bsss.archive
