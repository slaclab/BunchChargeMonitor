...
cat $IOC_DATA/$1/iocInfo/IOC.pvlist | grep -v "wave" | grep -v 'BSSSCHNMASK\|BSSSCHNSEVR\|SCUD1\|PIDSCUD1\|SCUD2\|PIDSCUD2\|SCUD3\|PIDSCUD3\|SCS1H\|PIDSCS1H\|SCSTH\|PIDSCSTH\|SCSHH\|PIDSCSHH\|SCH1H\|PIDSCH      1H\|SCHTH\|PIDSCHTH\|SCHHH\|PIDSCHHH' | grep -v string | grep -v subArray | grep -v sub | grep -v bsa | grep -v compress | grep -v fanout | grep -v seq | cut -f1 -d "," | sed 's/$/ 10 scan/' > $IOC_DATA/$1/archive/MAIN.archive
