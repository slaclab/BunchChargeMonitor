...
def makeTemplate(ai, longin, longout, bi, bo, output, base, devtype, revision, extraPdos):
    ...
    for pdo in extraPdos:
        pdolist = pdolist + f"pdo:#x"


    with open(output, "w") as file_object:
        macro_header = f"""
# template generated by maketemplate.py 
# from XML files at {base}
# for device {devtype} revision {revision:#x} 
# non-default pdo assignments: {extraPdos}
#
# %% macro, DEVICE, device name
# %% macro, PORT,   asyn port for device
# %% macro, SCAN,   scan rate
"""
        print(macro_header, file=file_object)
        for l in ["AL_STATE"]:
            al_text = f'''
record(mbbi, "$(DEVICE):{fixname(l)}")
{{
    field("DTYP", "asynInt32")
    field("INP",  "@asyn($(PORT)){l}")
    field("SCAN", "$(SCAN)")
    field("ONVL", "1")
    field("TWVL", "2")
    field("THVL", "4")
...
'''
            ...
            print(al_text, file=file_object)
        for l in ["ERROR_FLAG"]:
            longin_text = f"""
record(longin, "$(DEVICE):{fixname(l)}")
{{
    field("DTYP", "asynInt32")
    field("INP",  "@asyn($(PORT)){l}")
    field("SCAN", "$(SCAN)")
...
"""
            print(longin_text, file=file_object)
        for l in longin:
            longin_text = f"""
record(longin, "$(DEVICE):{fixname(l)}")
{{
...
"""
            print(bi_text, file=file_object)
        for l in longout:
            longout_text = f"""
record(longout, "$(DEVICE):{fixname(l)}")
{{
    field("DTYP", "asynInt32")
    field("OUT",  "@asyn($(PORT)){l}")
    field("OMSL", "supervisory")
}}
"""
            print(longout_text, file=file_object)
        for l in bo:
            bo_text = f"""
record(bo, "$(DEVICE):{fixname(l)}")
{{
    field("DTYP", "asynInt32")
    field("OUT",  "@asyn($(PORT)){l}")
    field("OMSL", "supervisory")
...
"""
            print(bo_text, file=file_object)
        for l in ai:
            ai_text = f"""
record(ai, "$(DEVICE):{fixname(l)}")
{{
...
"""
            print(ai_text, file=file_object)
...
def parseFile(filename, output, list_devices, extraPdos):
    ...
    for device in doc.xpathEval("//Device"):
        ...
        if list_devices:
            print(f"{devtype} {revision:#x} (product = {product:#x})")
            continue
            ...
            for txpdo in device.xpathEval("TxPdo"):
                ...
                for entry in txpdo.xpathEval("Entry"):
                    ...
                    if hasEntryName(entry):
                        ...
                        print(f"Ignoring entry in pdo {txpdo}")
...
if __name__ == "__main__":
    ...
    if not list_devices:
        ...
        if verbose:
            for obj in reqs:
                print(f"Searching device {obj}, revision {obj:#x}")
